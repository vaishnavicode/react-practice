import React, { useEffect, useState } from "react";
import Display from "./Display";

var users_values = [
  {
    firstName: "John",
    lastName: "Doe",
    email: "john.doe@example.com",
    phone: "123-456-7890",
    dob: "1990-01-15",
    age: 0,
    profileImageUrl: "https://randomuser.me/api/portraits/men/1.jpg",
    isAdmin: false,
    isDeleted: false,
    address: "123 Main St, New York, NY 10001",
  },
  {
    firstName: "Jane",
    lastName: "Smith",
    email: "jane.smith@example.com",
    phone: "234-567-8901",
    dob: "1985-05-20",
    age: 0,
    profileImageUrl: "https://randomuser.me/api/portraits/women/2.jpg",
    isAdmin: true,
    isDeleted: false,
    address: "456 Oak St, San Francisco, CA 94102",
  },
  {
    firstName: "Michael",
    lastName: "Johnson",
    email: "michael.johnson@example.com",
    phone: "345-678-9012",
    dob: "1992-08-25",
    age: 0,
    profileImageUrl: "https://randomuser.me/api/portraits/men/3.jpg",
    isAdmin: false,
    isDeleted: false,
    address: "789 Pine St, Los Angeles, CA 90003",
  },
  {
    firstName: "Emily",
    lastName: "Davis",
    email: "emily.davis@example.com",
    phone: "456-789-0123",
    dob: "1994-12-30",
    age: 0,
    profileImageUrl: "https://randomuser.me/api/portraits/women/4.jpg",
    isAdmin: true,
    isDeleted: true,
    address: "101 Maple St, Chicago, IL 60604",
  },
  {
    firstName: "David",
    lastName: "Martinez",
    email: "david.martinez@example.com",
    phone: "567-890-1234",
    dob: "1988-03-10",
    age: 0,
    profileImageUrl: "https://randomuser.me/api/portraits/men/5.jpg",
    isAdmin: false,
    isDeleted: false,
    address: "202 Birch St, Houston, TX 77005",
  },
  {
    firstName: "Olivia",
    lastName: "Garcia",
    email: "olivia.garcia@example.com",
    phone: "678-901-2345",
    dob: "1995-06-15",
    age: 0,
    profileImageUrl: "https://randomuser.me/api/portraits/women/6.jpg",
    isAdmin: false,
    isDeleted: true,
    address: "303 Cedar St, Phoenix, AZ 85006",
  },
  {
    firstName: "James",
    lastName: "Rodriguez",
    email: "james.rodriguez@example.com",
    phone: "789-012-3456",
    dob: "1991-09-20",
    age: 0,
    profileImageUrl: "https://randomuser.me/api/portraits/men/7.jpg",
    isAdmin: true,
    isDeleted: false,
    address: "404 Elm St, Philadelphia, PA 19107",
  },
  {
    firstName: "Sophia",
    lastName: "Wilson",
    email: "sophia.wilson@example.com",
    phone: "890-123-4567",
    dob: "1987-11-05",
    age: 0,
    profileImageUrl: "https://randomuser.me/api/portraits/women/8.jpg",
    isAdmin: false,
    isDeleted: false,
    address: "505 Spruce St, San Antonio, TX 78208",
  },
  {
    firstName: "Daniel",
    lastName: "Anderson",
    email: "daniel.anderson@example.com",
    phone: "901-234-5678",
    dob: "1983-04-18",
    age: 0,
    profileImageUrl: "https://randomuser.me/api/portraits/men/9.jpg",
    isAdmin: false,
    isDeleted: true,
    address: "606 Walnut St, Dallas, TX 75209",
  },
  {
    firstName: "Mia",
    lastName: "Thomas",
    email: "mia.thomas@example.com",
    phone: "012-345-6789",
    dob: "1999-07-25",
    age: 0,
    profileImageUrl: "https://randomuser.me/api/portraits/women/10.jpg",
    isAdmin: true,
    isDeleted: false,
    address: "707 Chestnut St, San Jose, CA 95110",
  },
  {
    firstName: "William",
    lastName: "Taylor",
    email: "william.taylor@example.com",
    phone: "123-456-7891",
    dob: "1993-02-14",
    age: 0,
    profileImageUrl: "https://randomuser.me/api/portraits/men/11.jpg",
    isAdmin: false,
    isDeleted: false,
    address: "808 Redwood St, Austin, TX 78711",
  },
  {
    firstName: "Isabella",
    lastName: "Harris",
    email: "isabella.harris@example.com",
    phone: "234-567-8902",
    dob: "1996-10-12",
    age: 0,
    profileImageUrl: "https://randomuser.me/api/portraits/women/12.jpg",
    isAdmin: true,
    isDeleted: false,
    address: "909 Willow St, Jacksonville, FL 32212",
  },
  {
    firstName: "Alexander",
    lastName: "Lee",
    email: "alexander.lee@example.com",
    phone: "345-678-9013",
    dob: "1982-01-28",
    age: 0,
    profileImageUrl: "https://randomuser.me/api/portraits/men/13.jpg",
    isAdmin: false,
    isDeleted: true,
    address: "1010 Sycamore St, Indianapolis, IN 46213",
  },
  {
    firstName: "Ava",
    lastName: "Walker",
    email: "ava.walker@example.com",
    phone: "456-789-0124",
    dob: "1997-05-30",
    age: 0,
    profileImageUrl: "https://randomuser.me/api/portraits/women/14.jpg",
    isAdmin: false,
    isDeleted: false,
    address: "1111 Poplar St, Columbus, OH 43214",
  },
  {
    firstName: "Henry",
    lastName: "Young",
    email: "henry.young@example.com",
    phone: "567-890-1235",
    dob: "1986-12-19",
    age: 0,
    profileImageUrl: "https://randomuser.me/api/portraits/men/15.jpg",
    isAdmin: true,
    isDeleted: false,
    address: "1212 Fir St, Fort Worth, TX 76115",
  },
  {
    firstName: "Charlotte",
    lastName: "King",
    email: "charlotte.king@example.com",
    phone: "678-901-2346",
    dob: "1998-07-21",
    age: 0,
    profileImageUrl: "https://randomuser.me/api/portraits/women/16.jpg",
    isAdmin: false,
    isDeleted: true,
    address: "1313 Pine St, Charlotte, NC 28216",
  },
];

const AddUser = () => {
  const [firstName, setFirstName] = useState("");
  const [lastName, setlastName] = useState("");
  const [email, setEmail] = useState("");
  const [phoneNumber, setPhoneNumber] = useState("");
  const [dob, setDob] = useState("");
  const [profileImageUrl, setProfileImageUrl] = useState("");
  const [address, setAddress] = useState("");
  const [users, setUsers] = useState(users_values);
  const [history, setHistory] = useState([]);

  const addUser = () => {
    var values = String(
      firstName,
      lastName,
      email,
      phoneNumber,
      dob,
      profileImageUrl,
      address
    );
    console.log(values);

    if (values !== "") {
      users_values.push({
        firstName: firstName,
        lastName: lastName,
        email: email,
        phone: phoneNumber,
        dob: dob,
        age: 0,
        profileImageUrl: profileImageUrl,
        isAdmin: false,
        isDeleted: false,
        address: address,
      });
      setUsers(users_values);
      localStorage.setItem("users", users);
    }
  };

  const addToHistory = (idGettingEdited) => {
    var toBeHistory = users.find(
      (val) => val.firstName + " " + val.lastName === idGettingEdited
    );

    setHistory((prevState) => {
      var x = prevState;
      x.push(JSON.stringify(toBeHistory));
      return x;
    });
    console.log(history);
  };
  const editUser = (
    idGettingEdited,
    firstName,
    lastName,
    email,
    phoneNumber,
    dob,
    profileImageUrl,
    address
  ) => {
    users.map((value, index) => {
      if (value.firstName + " " + value.lastName === idGettingEdited) {
        value.firstName = firstName !== "" ? firstName : value.firstName;
        value.lastName = lastName !== "" ? lastName : value.lastName;
        value.email = email !== "" ? email : value.email;
        value.dob = dob !== "" ? dob : value.dob;
        value.phone = phoneNumber !== "" ? phoneNumber : value.phone;
        value.profileImageUrl =
          profileImageUrl !== "" ? profileImageUrl : value.profileImageUrl;
        value.address = address !== "" ? address : value.address;
      }
      return index;
    });
  };

  useEffect(() => {
    console.log("Use Effect called");
  }, []);

  return (
    <React.Fragment>
      <div className="heading">
        <h1>Add User</h1>
      </div>
      <div className="forms">
        <form>
          <label className="add-user-label">
            Enter your first name
            <input
              type="text"
              name="firstName"
              onChange={(e) => setFirstName(e.target.value)}
            />
          </label>
          <br />
          <label className="add-user-label">
            Enter your last name
            <input
              type="text"
              name="lastName"
              onChange={(e) => setlastName(e.target.value)}
            />
          </label>
          <br />
          <label className="add-user-label">
            Enter your email
            <input
              type="email"
              name="email"
              onChange={(e) => setEmail(e.target.value)}
            />
          </label>
          <br />
          <label className="add-user-label">
            Enter your phone number
            <input
              type="text"
              name="phoneNumber"
              onChange={(e) => setPhoneNumber(e.target.value)}
            />
          </label>
          <br />
          <label className="add-user-label">
            Enter your date of birth
            <input
              type="date"
              name="dob"
              onChange={(e) => setDob(e.target.value)}
            />
          </label>
          <br />
          <label className="add-user-label">
            Url of your profile picture
            <input
              type="url"
              name="profileImageUrl"
              onChange={(e) => setProfileImageUrl(e.target.value)}
            />
          </label>
          <br />
          <label className="add-user-label">
            Address
            <input
              type="text"
              name="address"
              maxLength={255}
              onChange={(e) => setAddress(e.target.value)}
            />
          </label>
          <br />
          <button type="button" onClick={addUser} className="add-user-button">
            Add User
          </button>
        </form>
      </div>
      <Display users={users} editUser={editUser} addToHistory={addToHistory} />
    </React.Fragment>
  );
};

export default AddUser;
